...
public class QuestionService(NpgsqlConnection connection) : IQuestionService, IDisposable
{
    ...
    public async Task<bool> Create(Question question) {
        ...
        await connection.CloseAsync();
        var insertItemQuery = "INSERT INTO pergunta_itens (pergunta_id, pergunta, tipo, quantidade_campos, obj_options, subj_unico_val, subj_duplo_val) ";
        foreach (QuestionItem item in question.Items) {
            insertItemQuery += $"VALUES ({newQuestionId}, '{item.QString}', '{item.QType}', '{item.Quantity}', '{item.ObjOptions}', '{item.SubjSingleValue}', '{item.SubjDoubleValue}') ";
        }
        using var cmd1 = connection.CreateCommand();
